...
- (void)setURLArray:(NSArray*) data
{
    ...
    if(data&&data.count>0){
        NSURL *videoURL = [NSURL URLWithString:self.urlArray[0]];
        [super setContentURL:videoURL];
        [self play];
    }
}
...
- (void)dismiss
{
    ...
    [UIView animateWithDuration:kVideoPlayerControllerAnimationTimeInterval animations:^{
        ...
        if (self.kRVideoPlayBlock) {
            self.kRVideoPlayBlock(KRVideoPlayClose,0);
        }
    }];
...
- (void)configControlAction
{
    ...
    [self.videoControl.preButton addTarget:self action:@selector(preButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [self.videoControl.nextButton addTarget:self action:@selector(nextButtonClick) forControlEvents:UIControlEventTouchUpInside];
...
{
    self.urlIndex++;
    if(self.urlArray && (self.urlIndex < self.urlArray.count)&& self.urlIndex >= 0){
        NSURL *videoURL = [NSURL URLWithString:self.urlArray[self.urlIndex]];
        [super setContentURL:videoURL];
        if (self.kRVideoPlayBlock) {
            self.kRVideoPlayBlock(KRVideoPlayNext,self.urlIndex);
        }
        [self play];
    }else{
        self.urlIndex = 0;
